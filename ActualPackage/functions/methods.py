...
def batchRun(model, trainSet, originalDic, testSet, num_of_centroids, factors, log, epochs = 40, random = 6, MAE = 1, RMSE = 1 ): 
...
def totalRun(fileName, model, startYear, min_NO_rating, totalNOB, cluster_size,\
     batch_size, num_of_centroids, factors, maxEpochs = 40, Random = 6, mae = True, rmse = True):
    ...
    for XthBatch in range(1,totalNOB+1):
        ...
        batchRun(model, trainSet, originalDic, testSet, num_of_centroids, factors, log, epochs = maxEpochs, random = Random, MAE = mae, RMSE = rmse )
    log.close
