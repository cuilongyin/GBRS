...
def prpareTrainTestObj(df, batchDic_unCluster,valiDic, batch_size, NOofBatches, windowSize):
    ...
    df_train, df_test, df_validate = furtherFilter(2 ,df_train, df_test, df_validate)
...
for XthBatch in range(1, totalNOB+1):                  
    ...
    df_train['user_id'].drop_duplicates().to_csv(f"./DataPreProcessing/batchFiles/{XthBatch}_uc.userlist", index=False, header=False)
    df_train['bus_id'].drop_duplicates().to_csv(f"./DataPreProcessing/batchFiles/{XthBatch}_uc.itemlist", index=False, header=False)
    df_train[['user_id', 'bus_id', 'rating']].to_csv(f"./DataPreProcessing/batchFiles/{XthBatch}_uc.train.rating", index=False, header=False)
    df_test[['user_id', 'bus_id', 'rating']].to_csv(f"./DataPreProcessing/batchFiles/{XthBatch}_uc.test.rating", index=False, header=False)
    df_validate[['user_id', 'bus_id', 'rating']].to_csv(f"./DataPreProcessing/batchFiles/{XthBatch}_uc.valid.rating", index=False, header=False)
    df_all = pd.concat([df_train, df_test]) 
    df_all.to_csv(f"./DataPreProcessing/batchFiles/{XthBatch}_uc.all.rating", index=False, header=False)